...
class MotorTorque_and_Jtsbiased_data(Node):
    ...
    def start_logging(self, task_number):
        ...
        print(f"\n---> Started recording continuous data to: {file_name}")
    ...
    def timer_loop(self): 
        ...
        while rclpy.ok(): 
            ...
            if self.is_logging and self.motor_data_writer:
                # Get precise timestamp including milliseconds
                timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]     
                
                # Ensure lists aren't empty/None to prevent iteration errors
...
def main():
    ...
    try:
        ...
        # Ensure we close the file if aborted mid-task
        node.stop_logging() 
